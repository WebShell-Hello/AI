...
# Description: K-means++ smart initialization; This is the main difference between K-means++ and standard K-means
# Logic: Uses probability weighting to give points farther from current centroids higher probability of being selected as next centroid
def initialSelection(x, k):
    centroids = []
    # 1. Randomly select first centroid
    first_idx = np.random.randint(x.shape[0])
    centroids.append(x[first_idx])
    # 2. Select remaining k-1 centroids
    for _ in range(1, k):
        # Calculate distance from each point to its nearest centroid
        distances = np.array([min([np.linalg.norm(point - c) for c in centroids]) for point in x])
        # Convert distances to selection probabilities
        probabilities = distances ** 2 / np.sum(distances ** 2)
        # Select next centroid according to probabilities
...
def main():
    ...
    title = 'Task3: k-Means++ Clusters Algorithm'
    savefig = 'Task3_k-Means++_Silhouette_coefficient.png'
